...
def parse_rpg(rpg_file, label_names=['CODE_GROUP']):
    ...
    print('CONTENT: ', data['features'][0])
    for f in tqdm(data['features']):
        p = Polygon(f['geometry']['coordinates'][0][0]) 
        polygons[f['properties']['parcela']] = p # Asocia al id de la parcela las coordenadas del recinto

        for l in label_names:
            lab_rpg[l][f['properties']['parcela']] = f['properties'][l] # probado con idPanel no funciona
...
def get_dates(input_folder):
    ...
    tifs =  np.sort(tifs)
    dates = [int(t.replace('.tif', '').replace('-', '')) for t in tifs]
    ndates = len(dates)
...
if __name__ == '__main__':
    rpg_file = 'shapefiles/tabuyo/tabuyo_2023/mini_tabuyo.geojson'
    input_folder = 'datacubes/tabuyo/2023/mini_tiffs_4326'
    out_path = 'datacubes/tabuyo/2023/mini_dataset'

    prepare_dataset(out_path, input_folder, rpg_file, label_names=['uso_sigpac'])
